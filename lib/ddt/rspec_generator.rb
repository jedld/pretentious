class Ddt::RspecGenerator
  ...
  def generate(test_instance)
    if (test_instance.is_a? Class)
      ...
      buffer(declare_dependencies(args, test_instance.init_let_variables, 2))

      if (args.size > 0)
        buffer("@fixture = #{test_instance.test_class.name}.new(#{params_generator(args, test_instance.init_let_variables)})",2)
      else
